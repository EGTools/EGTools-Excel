...
<ul>
   ...
   <br>
   <li> 다단계 선태 기준 : 다단계 유효성 검사의 목록(Dropdown List)을 생성</li>
   <li> 다단계 선태 적용 : 선택한 셀에 다단계 유효성 검사를 적용</li>
   <li> 다단계 선태 삭제 : 필요없는 다단계 유효성 검사를 삭제</li>
   <br>
...
## EG Tools 전용 UDF
<ul>
   <li>KoreanHolidays : 지정하는 연도의 대한민국 공휴일 목록을 출력 </li>
   <li>DateTime : 문자열에서 날짜와 시간을 찾아서 날짜와 시간형식으로 변환하는 함수 </li>
   <li>DisplayColor : 대상 셀의 조건부 서식을 포함하여 실제로 표시된 바탕색/글자색의 번호 (검정=0 ~ 흰색=16777215) </li>
